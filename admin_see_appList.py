...
class WindowForAdminSeeAppList():
    ...
    def __init__(self, adminsee,val30):
        ...
        # find all the pt appointments,including past and upcoming ones
        conn_admin_see = sqlite3.connect('Database.db')
        c_admin_see = conn_admin_see.cursor()

        re_app = c_admin_see.execute("SELECT appID,appointmentTime,isApproved,DrName FROM appointments WHERE ptID = (?)"
                                     ,(self.ptID,))
        isApp = ''
